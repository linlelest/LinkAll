...
- [ ] `android/` 工程存在，Kotlin 2.1.20 + Compose 1.7.2 + Koin 依赖注入配置完成
- [ ] 三页签导航实现（被控/控制/管理），主题跟随系统深色模式，中英文热切换无需重启
- [ ] 禁用 Jetpack Navigation，使用简单状态管理
- [ ] MediaProjection 录屏流 + libwebrtc（m142+）编码推送管道实现
- [ ] AccessibilityService 全局键鼠事件注入与拦截实现
- [ ] 前台保活服务 + 通知栏常驻快捷开关实现（断开/防窥屏/文件接收开关）
- [ ] 首次权限引导页实现（无障碍/后台弹出/电池优化白名单/开机自启，适配各厂商）
- [ ] 本地设置界面实现（登录/退出、Token 加密存储、安全设置、重置编号/设备码、退出清理）
- [ ] 连接确认弹窗实现（匿名请求全屏遮罩、显示请求端 IP/ID/设备名、[允许一次/永久允许/拒绝/设备码放行]）
- [ ] 连接页实现（设备编号/密码、模式切换、同账号自动发现设备列表）
- [ ] 控制页全沉浸式实现（SurfaceView/ExoPlayer 渲染 + 双指捏合缩放）
- [ ] 虚拟工具层实现（自定义 View：虚拟键盘输入法接管、虚拟鼠标 MotionEvent 映射、左右键悬浮球、滚轮侧边条）
- [ ] 手势冲突处理实现（边缘滑动呼出设置面板）
- [ ] 参数面板底部抽屉实现（缩放/码率/帧率滑块、防窥屏开关、文件传输入口）
- [ ] SAF 文件传输实现（双向、进度通知、断点续传、外部 SD 卡读写）
- [ ] 管理端用户中心实现（账号信息、邀请码管理、全局安全开关、自定义服务器覆盖）
- [ ] 公告中心实现（下拉刷新、compose-markdown MD 渲染、图片点击放大）
- [ ] 设备列表实现（在线/离线状态、一键连接、远程踢出、查看设备信息）
- [ ] OTA 更新实现（后台静默检查、前台弹窗下载、FLAG_INSTALL 引导安装、强制更新拦截返回键）
- [ ] Android 14+ 后台限制保活策略实现 + 自启动引导（各厂商 ROM 适配）
- [ ] 内存占用 <60MB，冷启动 <0.8s
- [ ] `.github/workflows/android.yml` 存在，配置 push 到 main + 路径 `android/**` 触发
- [ ] Phase 4 已 push 至 origin/main，GitHub Actions Android 构建 workflow 运行成功，arm64-v8a + armeabi-v7a APK（R8 全开）artifact 上传成功
