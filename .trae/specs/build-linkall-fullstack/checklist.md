...
- [x] WebRTC DataChannel 文件分片协议实现（单包 256KB、分片 ID+偏移量校验）
- [x] SHA-256 哈希校验完整性实现
- [x] 断点续传实现（记录至本地 DB）与传输队列管理实现
- [x] 网页端文件管理器实现（目录树、上传/下载队列、路径选择、传输统计）
- [x] Android 端 SAF 文件管理器完善
- [x] 匿名连接流程实现（编号输入 + 设备端手动确认 单次/永久/拒绝/设备码放行）
- [x] 同账号连接流程实现（设备自动发现 + 首次配对确认 + 免密直连）
- [x] 全局安全策略实现（匿名/设备码/连接总开关服务端校验与实时同步）
- [x] 自定义服务器地址覆盖热重载实现
- [x] 控制指令时间戳防重放 + 设备码/编号非对称加密传输 + 本地配置 0600 权限实现
- [x] 公告系统服务端实现（Markdown 解析、富文本工具栏、多条置顶、按平台/版本推送、阅读状态追踪、数字签名）
- [x] 客户端分级日志实现（DEBUG/INFO/WARN/ERROR，一键打包导出）
- [x] 崩溃捕获实现（Rust backtrace / Android 自研轻量 Crashlytics）上传至 /crash
- [x] 服务端结构化日志实现（JSON 格式）
- [x] Phase 5 已 push 至 origin/main，各端 CI workflow 运行成功
